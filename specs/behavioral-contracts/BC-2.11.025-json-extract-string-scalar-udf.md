...
- **Plan-time literal-key gate (ADR-066 §B3):** the `check_json_extract_key_literal` gate fires at plan time (after plan-time gates E-QUERY-037/038/039, before DataFusion execution/fan-out). E-QUERY-041/042 temporal validation runs in-pipeline per ADR-052 §D4 (inside `run_materialization_pipeline`, after the plan-time gate sequence) — these are not part of the pre-execution plan-gate ordering and no specific ordering between E-QUERY-045 and in-pipeline temporal validation is asserted beyond ADR-066 §B3 ("before DataFusion execution"). Sub-case (a): second argument is not `Expr::Literal(Literal::String(_))` → E-QUERY-045(a). Sub-case (b): literal key exceeds 256 UTF-8 bytes → E-QUERY-045(b). No sensor fan-out occurs for a rejected query.
...
| EC-11-025-006 | Non-literal key at plan time: `json_extract_string(col, other_col)` where `other_col` is a column reference | Plan-time rejection with E-QUERY-045(a). Gate fires AFTER plan-time gates E-QUERY-037/038/039, BEFORE DataFusion execution; E-QUERY-041/042 run in-pipeline per ADR-052 §D4 (not part of this pre-execution plan-gate sequence). No fan-out occurs. MUST: S-JSON-EXTRACT-UDF-001 RG-JEX-006 (`test_jex_rg006_non_literal_key_rejected_e_query_045_a`). |
...
|---------|-------|------|--------|--------|
| 1.9 | beta3-errata1-spec | 2026-09-17 | product-owner | **LOW-1 (stale gate-ordering enumeration) + LOW-2 (inconsistent illustrative column).** **LOW-1:** §Postconditions §Plan-time literal-key gate and EC-11-025-006 both listed E-QUERY-041/042 in the pre-execution plan-gate ordering alongside E-QUERY-045. E-QUERY-041/042 temporal validation runs in-pipeline per ADR-052 §D4 (inside `run_materialization_pipeline`, after the plan-time gate sequence); listing them in the pre-execution sequence was stale. ADR-066 §B3 governs "before DataFusion execution" only; no specific ordering between E-QUERY-045 and in-pipeline temporal validation is asserted. **Fix:** §Postconditions updated to "after plan-time gates E-QUERY-037/038/039, before DataFusion execution/fan-out" with explicit note that E-QUERY-041/042 run in-pipeline per ADR-052 §D4 and are not part of the pre-execution plan-gate ordering. EC-11-025-006 updated identically. **LOW-2:** EC-11-025-011 illustrative query used `raw_payload`; `raw_extensions` is the canonical Tier-2 Arrow column per S-JSON-EXTRACT-UDF-001 §depends_on justification, AC-011, Problem Statement, and §Previous Story Intelligence (all anchored to S-ADR058-OCSF-ROUTING-001). **Fix:** all three `raw_payload` occurrences in EC-11-025-011 replaced with `raw_extensions`. No new MUSTs. **TD-VSDD-097 three-dimension discharge:** (1) Dim-1 sibling — no named split-event twin for BC-2.11.025; CLEAR. (2) Dim-2 downstream copy — `rg 'raw_payload' .factory/` returns only BC-2.11.025 EC-11-025-011 (no other downstream copy exists; fixed here). `rg 'E-QUERY-037/038/039/041/042/043' .factory/` returns story S-JSON-EXTRACT-UDF-001 AC-006 (line 233), T-09 (line 454), and Architecture Compliance Rules table (line 527) — these are downstream story copies of the BC postcondition; story-writer handles propagation per dispatch instruction (not edited by product-owner). (3) Dim-3 mandate anchor — text reconciliation only; no new MUSTs added or changed; CLEAR. BC-INDEX pin must be bumped v1.8→v1.9 (state-manager). |
