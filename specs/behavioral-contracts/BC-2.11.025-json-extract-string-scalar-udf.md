...
|-------|----------|-------------|
| VP-162 | `json_extract_string_impl` is a pure function: null/missing/parse-failure/non-object/JSON-null → `None`; string → `Some(s)`; non-string → `Some(value.to_string())`; formal bounds via structural `any_vec::<u8,256>()` key bound (ADR-066 §D1; VP-162 §Harnesses) | kani |
