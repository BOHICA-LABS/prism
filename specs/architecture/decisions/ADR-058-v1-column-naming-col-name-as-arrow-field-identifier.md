...
BC-2.01.013 §EC-01-025 (added D-924 burst, 2026-05-31) explicitly marks
...
`devices` table produces eight distinct flattened names after the §J3 and §K5 D-2522 fixes: `device_uid`,
`device_instance_uid`, `device_type_category`, `device_type_label`, `risk_score`, `device_is_online`,
...
| TOML spec-validity: `column_type = "boolean"` + `ocsf_field = "device.is_online"` → spec-engine processes without `ValidationError` (produces `ValidationWarning` for vendor-ext, non-blocking) | S-CLAROTY-OCSF-STATUS-001 RG-COS-006 |
| `WHERE device_is_online = true` / `= false` filters correctly at query layer (PrismQL Boolean predicate) | S-CLAROTY-OCSF-STATUS-001 RG-COS-007 |
| SAP-2 DTU parity: `ClarotyDevice.is_online: bool` on DTU wire → `device_is_online` Boolean column matches | S-CLAROTY-OCSF-STATUS-001 RG-COS-008 |
...
- **BC-2.01.013 §EC-01-025** (added D-924 burst 2026-05-31): marks "ColumnMapper step is
