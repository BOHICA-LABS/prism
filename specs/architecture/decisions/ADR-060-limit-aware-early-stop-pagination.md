...
| Meaning | JSON path (top-level key only (ADR-066 §D4), same literal-key rule (§D2) as `json_extract_string`) to an integer field in the API response body that carries the total upstream item count |
| Default | `None` — no upstream total is captured |
| Validation | No mechanized gate is enforced. `total_count_path` is operator-authored TOML; a key that does not resolve to a top-level integer in the response — absent, non-numeric, or extraction-failed — degrades gracefully: `upstream_total` remains `None` → `total_available = total_rows` (safe lower bound). No error is raised; the sensor behaves identically to one without `total_count_path` declared. Absent-path behavior: RG-QTT-006. Gated formula: RG-QTT-003. |

**Key-depth constraint:** `total_count_path` is a TOP-LEVEL key in the response JSON
object, not a JSONPath expression. The same key disciplines from ADR-066 apply —
literal-key (§D2) and top-level-only (§D4). §D4 governs the
no-nested-path constraint: values like `meta.total` or `pagination.count` are NOT
supported in beta.3 and are deferred to S-JSON-EXTRACT-NESTED-001's nested-path
mechanism. Sensors with nested total paths must wait for that story or restructure their
TOML wrapper.

**Query-filter scope constraint:** `total_count_path` SHOULD be declared only when the
sensor's total count field reflects the APPLIED query filters — i.e., the sensor API
returns a count scoped to the current query/filter context, not a grand unfiltered table
total. Examples of compliant usage: Claroty's `/assets?page=N&per_page=K` responses
include `{"total": M}` where M reflects the filtered result set. If a sensor API always
returns the full unfiltered table count regardless of query parameters, declaring
`total_count_path` is NOT incorrect (the hard `total_available >= total_rows` invariant
prevents harmful under-reporting), but the `total_available` field in the MCP response
will overstate the true query-scoped count for filtered queries. Sensor operators SHOULD
document this behavior in a TOML comment when applicable. Example:

```toml
[[tables]]
name = "devices"
# NOTE: API returns unfiltered grand total; total_available overstates for filtered queries.
total_count_path = "total"
```

#### §D8.11.2 — PaginationCursor.total_count (Existing Field, New Consumer)

`PaginationCursor` carries `total_count: usize` (at
`crates/prism-sensors/src/pagination.rs`). This field is initialized to `usize::MAX` as a
"not-yet-received" sentinel by `PaginationCursor::new()`; each call to
`PaginationCursor::advance()` sets it to the API-reported total for that page. When
`total_count_path` is set in the TOML, the adapter populates this field on every page;
without `total_count_path`, the field remains at `usize::MAX` throughout pagination.

The pipeline maps the sentinel to `None` and any real value to `Some(n)` when surfacing
`upstream_total` in `FetchOutput` (§D8.11.3). The sentinel guard —
`if raw == usize::MAX { None } else { Some(raw) }` — is applied in `spec_driven_adapter.rs`
(S-QUERY-TRUE-TOTAL-001 T-B03) to prevent propagating the initialization sentinel as an
apparent API-reported total.
...
via the `FetchOutput.upstream_total` field (§D8.11.3), applying the sentinel guard above.
...
**Client-side WHERE filters also suppress early-stop:** When `has_client_side_where`
returns `true` (§D8.7), early-stop is suppressed for that query — `any_early_stopped`
is `false` even if the query is a straight projection with a WHERE predicate. This means
`total_available = total_rows` for filtered straight projections, which is the correct
conservative lower-bound behavior. No separate gate is needed: the existing
`any_early_stopped` proxy already captures "no WHERE filter that could reduce the result
set." The `upstream_total` override applies only when early-stop was truly triggered.

**Conservative under-report: `any_pipeline_truncated=true` / `any_early_stopped=false`:**
When the DI-019 capacity cap fires (`any_pipeline_truncated = true`) without a LIMIT
early-stop (`any_early_stopped = false`), `total_available = total_rows` — the capped
row count, NOT the true upstream count. This is intentionally conservative: the DI-019
cap is an independent safety net that applies to ALL queries (not only LIMIT queries),
and the `total_count_path` mechanism is designed for LIMIT-early-stop scenarios only.
Extending `upstream_total` to DI-019-capped results is out of scope for beta.3 and
deferred to a post-beta enhancement with its own story anchor.
...
| Single-sensor with `total_count_path`: MCP response field `total_available` reflects the sensor's upstream total (via `upstream_total.map(|n| n.max(total_rows))`) when query early-stopped and `upstream_total ≥ total_rows` | S-QUERY-TRUE-TOTAL-001 RG-QTT-004 |
...
| `is_truncated` formula UNCHANGED by `upstream_total`: `is_truncated = (total_rows > limit) \|\| any_early_stopped \|\| any_pipeline_truncated`; upstream_total does NOT affect `is_truncated` | S-QUERY-TRUE-TOTAL-001 RG-QTT-010 |
| Multi-sensor mixed `total_count_path`: one sensor declares `total_count_path`, another does not → `FanOutResult.upstream_total = Some(N)` from the sensor that declares it; `total_available` uses that value when `any_early_stopped = true` | S-QUERY-TRUE-TOTAL-001 RG-QTT-011 |
...
|---------|------|--------|--------|
| 1.28 | 2026-09-17 | architect | Corrective amendment: §D8.11.2 type correction — `PaginationCursor.total_count` is `total_count: usize` (initialized to `usize::MAX` as "not-yet-received" sentinel by `PaginationCursor::new()`; set to API-reported total by `PaginationCursor::advance()`), NOT `Option<usize>` as the previous text claimed. Pipeline maps the sentinel to `None` and any real value to `Some(n)` when surfacing `upstream_total` in `FetchOutput` (§D8.11.3); sentinel guard `if raw == usize::MAX { None } else { Some(raw) }` applied in `spec_driven_adapter.rs` per S-QUERY-TRUE-TOTAL-001 T-B03. §D8.11.6 UNCHANGED — the mandate table and RG-QTT-001..011 anchors are unaffected by this type correction; downstream story re-pin of ADR-060 anchor is a version-only bump. TD-VSDD-097 three-dimension discharge: (1) Dim-1 sibling — BC-2.11.001 EC-11-095 test vector carries `PaginationCursor.total_count = Some(13000)` (wrong type notation); routed to product-owner; this ADR edit does NOT touch BC-2.11.001. (2) Dim-2 downstream copy — §D8.11.2 type claim was sourced into BC-2.11.001 and S-QUERY-TRUE-TOTAL-001; story T-B03 sentinel guard already reflects the correct type; BC product-owner propagation required. (3) Dim-3 mandate anchor — no new MUST added; existing "must NOT discard" retained, already anchored at §D8.11.6 RG-QTT-001. ARCH-INDEX not touched (state-manager close-out). |
| 1.27 | 2026-09-16 | architect | Re-gate pass-15 fix F-B0P15-001 (MED): §D8.11.1 Key-depth constraint — removed the erroneous `length-bounded (§D3)` clause (ADR-066 §D3 is the query-time key-length gate, inapplicable to operator-authored `total_count_path` which has no mechanized gate per the Validation row); now cites §D2 (literal-key) + §D4 (top-level-only) only, matching the Meaning cell and Validation row. POL-29 dim-1 sibling-sweep residual from the v1.26 §D4 re-anchor. Re-freeze pending BC-5.39.001 3-CLEAN. |
| 1.26 | 2026-09-16 | architect | Re-gate pass-13 fix F-B0P13-LOW-001 (LOW): §D8.11.1 Key-depth constraint re-anchored — no-nested-path constraint + S-JSON-EXTRACT-NESTED-001 deferral now cite ADR-066 §D4 (was §D2/§D3); §D2/§D3 retained for literal-key/length disciplines; Meaning cell top-level-key anchored to §D4. Re-freeze pending BC-5.39.001 3-CLEAN. |
| 1.25 | 2026-09-16 | architect | Re-gate pass 10 fix. F-2 (LOW, pending-intent): `inputs:` block and `input-hash: "pending"` added to frontmatter — convention verification confirmed 14 sibling ADRs carry `input-hash:` (ADR-003, ADR-022, ADR-023, ADR-026, ADR-029, ADR-030, ADR-039, ADR-055, ADR-056, ADR-057, ADR-058, ADR-063, ADR-064, ADR-066); ADR-060 was the only delta ADR without it. `inputs:` populated from S-ENGINE-LIMIT-EARLY-STOP-001 file context: `crates/prism-spec-engine/src/pipeline.rs` (PaginationCursor, FetchContext) and `crates/prism-query/src/materialization.rs` (engine Step 6, total_available). `input-hash: "pending"` matches ADR-066's posture (hash to be computed when delta ADR batch stabilizes). §Status heading currency-synced v1.24→v1.25. |
| 1.24 | 2026-09-16 | architect | Re-gate pass 9 fix. F-3 (LOW): §D8.11.1 query-filter scope constraint — unanchored operator MUST softened to SHOULD. "Sensor operators MUST document this behavior in a TOML comment when applicable" was a documentation directive with no corresponding code gate or RG-QTT anchor; changed to SHOULD, consistent with the existing SHOULD in the same paragraph and with TD-VSDD-097 Dim-3 (no unanchored MUST). No gate or Red Gate added. |
| 1.23 | 2026-09-16 | architect | Re-gate pass 8 fix. F-1 (LOW): §D8.11.1 Validation row softened — removed borrowed "≤256 bytes / CWE-400" framing (no mechanized gate exists for key length); removed unanchored "non-negative integer" MUST; replaced with accurate graceful-degradation behavior: a key that does not resolve (absent, non-numeric, extraction-failed) leaves `upstream_total = None` → `total_available = total_rows` (safe lower bound), no error raised. Anchors: absent-path = RG-QTT-006; formula = RG-QTT-003. No new gate or Red Gate added (operator-authored TOML, non-harmful degradation). TD-VSDD-097: (1) sibling pair — no twin ADR; CLEAR. (2) downstream copy target — §D8.11.1 Validation cell is not independently copied; §D8.11.5/§D8.11.6 unchanged. (3) mandate anchor — no new MUST; existing RG-QTT-003/006 cited as anchors for descriptive behavior. |
| 1.22 | 2026-09-16 | architect | Re-gate pass 2 fix. OBS-3 (LOW): §D8.11.6 RG-QTT-004 MUST text reworded — removed "upstream_total = Some(N) present in MCP response" (internal struct field, not MCP-visible); replaced with "MCP response field `total_available` reflects the sensor's upstream total (via `upstream_total.map(|n| n.max(total_rows))`) when query early-stopped and `upstream_total >= total_rows`". Now references the MCP-surface field `total_available` as the observable, consistent with BC-2.11.001 MUST-4. |
| 1.21 | 2026-09-16 | architect | Re-gate pass 1 fixes. F-7 (MED): §D8.11.1 query-filter scope constraint added — `total_count_path` SHOULD be declared only when sensor total reflects applied query filters; if sensor returns unfiltered grand total, operator MUST document overstatement risk in TOML comment. OBS-1 (LOW): §D8.11.5 rationale extended — client-side WHERE filters suppress early-stop via `has_client_side_where` gate (§D8.7), making `any_early_stopped` a sound proxy for no-row-reduction WITH no filter suppression; no separate gate needed. OBS-2 (LOW): §D8.11.5 rationale extended — `any_pipeline_truncated=true`/`any_early_stopped=false` conservative under-report case documented; DI-019 cap fires independently; extending upstream_total to DI-019-capped results is out of scope for beta.3. §D8.11.6: RG-QTT-010 (is_truncated formula unchanged by upstream_total) and RG-QTT-011 (multi-sensor mixed total_count_path: Some(N) from declaring sensor, not None) added to mandate table. Finding-4 (OBS): §Status banner updated v1.19 → v1.21. |
